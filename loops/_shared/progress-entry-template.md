...
Tokens come from `shell.token_usage_command` in loops/config.md:
  total    = end.total - start.total (includes cache reads; sum over attempts)
  new work = delta(input) + delta(cache_creation) + delta(output)  (excludes cache reads)
Log BOTH numbers for every milestone (planning and every phase).
...
- **Duration:** {{hh:mm:ss}}
- **Tokens used:** total {{total}} · new work {{input + cache write + output}} ({{measured | estimated}}) — input {{n}} · cache write {{n}} · cache read {{n}} · output {{n}}
